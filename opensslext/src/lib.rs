...
pub mod ssl {
    ...
    fn tls13_extract_secrets(
        ...
        match (
            ...
            (Nid::AES_128_GCM, Nid::SHA256) => {
                let hkdf = Hkdf::<Sha256>::from_prk(traffic_secret).map_err(|_| {
                    "traffic secret is not the right length for HKDF-SHA256 PRK"
                })?;
            ...
            (Nid::AES_256_GCM, Nid::SHA384) => {
                let hkdf = Hkdf::<Sha384>::from_prk(traffic_secret).map_err(|_| {
                    "traffic secret is not the right length for HKDF-SHA384 PRK"
                })?;
